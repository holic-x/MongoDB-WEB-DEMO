  ...
  export default {
    ...
    methods: {
      ...
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/sys/comment/list'),
          method: 'post',
      ...
      deleteHandle (id) {
        var ids = id ? [id] : this.dataListSelections.map(item => {
          // return item.commentId 通过mangorepo操作方法返回对应映射为commentId
          return item._id // 通过自定义mongoTemplate操作方法返回对应comment文档实体映射为_id
        })
        this.$confirm(`确定对[id=${ids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
          ...
          this.$http({
            url: this.$http.adornUrl('/sys/comment/delete'),
            method: 'post',
            data: this.$http.adornData({commentIdList:ids}, false)
          }).then(({data}) => {
            if (data && data.code === 0) {
              this.$message({
                message: '操作成功',
